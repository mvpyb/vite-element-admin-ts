<template>
  <section class="todoapp">
    <header class="header">
      <input v-model="currentTodo" class="new-todo" autocomplete="off" placeholder="Todo List" @keyup.enter="addTodo" />
    </header>
...
export default {
  components : { Todo },
  setup() {
    const currentTodo = ref<string>( '' )
    const visibility = ref<string>( 'all' )
    const filters: Ref<filtersType> = ref( {
      all : todos => todos,
    ...
    const set: setType = reactive( {
      ...
      allChecked : computed( () => {
        return todos.value.every( ( todo: todoType ) => todo.done )
      } ),
      ...
      remaining : computed( () => {
        return todos.value.filter( ( todo: todoType ) => !todo.done ).length
      } )
    } )
    ...
    const deleteTodo = ( val: todoType ) => {
      todos.value.splice( todos.value.indexOf( val ), 1 )
      setLocalStorage()
    }

    const editTodo = ( params: { todo: todoType; value: string } ) => {
      const { todo, value } = params
      todo.text = value
      setLocalStorage()
    }

    const clearCompleted = () => {
      todos.value = todos.value.filter( ( todo: todoType ) => !todo.done )
      setLocalStorage()
    }

    const toggleAll = ( params: todoType ) => {
      const { done } = params
